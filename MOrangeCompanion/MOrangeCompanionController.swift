import AppKit
import QuartzCore

class MOrangeCompanionController {
    var characters: [WalkerCharacter] = []
    private var displayLink: CADisplayLink?
    private var audioCacheCleanupTimer: Timer?
    var debugWindow: NSWindow?
    var pinnedScreenIndex: Int = -1
    private static let onboardingKey = "hasCompletedOnboarding"

    private var pointerScreen: NSScreen? {
        let mouseLocation = NSEvent.mouseLocation
        return NSScreen.screens.first(where: { $0.frame.contains(mouseLocation) })
            ?? NSScreen.main
            ?? NSScreen.screens.first
    }


    func start() {
        let morange = WalkerCharacter(videoName: "morange-walk")
        morange.accelStart = 3.9
        morange.fullSpeedStart = 4.5
        morange.decelStart = 8.0
        morange.walkStop = 8.75
        morange.walkAmountRange = 0.35...0.6
        morange.minimumPauseRange = 0.05...0.15
        morange.restartWalkImmediately = true
        morange.animationKeywordMap = [
            (["跑", "run", "快跑", "冲", "走", "walk"], "morange-walk", true),
            (["思考", "想想", "thinking", "工作", "代码", "分析"], "morange-thinking", false),
            (["开工", "处理", "执行", "构建", "测试", "working", "build", "test"], "morange-working", false),
            (["开心", "高兴", "happy", "好耶", "完成", "搞定"], "morange-happy", false),
            (["完成啦", "done", "收工"], "morange-done", false),
            (["失败", "错误", "卡住", "不懂", "confused", "error"], "morange-confused", false),
            (["待机", "休息", "idle", "发呆", "陪", "贴贴"], "morange-idle", false),
            (["睡觉", "睡", "困", "sleepy"], "morange-sleepy", false),
            (["招财", "发财", "好运", "lucky", "money"], "morange-happy", false),
            (["委屈", "难过", "sad", "伤心"], "morange-thinking", false),
            (["害羞", "乖", "shy", "可爱"], "morange-idle", false)
        ]
        morange.animationTagItems = [
            ("走路", "morange-walk", true),
            ("待机", "morange-idle", false),
            ("思考", "morange-thinking", false),
            ("工作", "morange-working", false),
            ("完成", "morange-done", false),
            ("困惑", "morange-confused", false),
            ("困困", "morange-sleepy", false),
            ("开心", "morange-happy", false)
        ]
        morange.formPairMap = [
            "morange-walk": ("morange-idle", false),
            "morange-idle": ("morange-walk", true),
            "morange-thinking": ("morange-happy", false),
            "morange-working": ("morange-done", false),
            "morange-done": ("morange-working", false),
            "morange-confused": ("morange-thinking", false),
            "morange-sleepy": ("morange-idle", false),
            "morange-happy": ("morange-thinking", false)
        ]
        morange.animationScaleMap = [
            "morange-walk": 1.08,
            "morange-idle": 1.08,
            "morange-thinking": 1.08,
            "morange-working": 1.08,
            "morange-done": 1.08,
            "morange-confused": 1.08,
            "morange-sleepy": 1.08,
            "morange-happy": 1.08
        ]
        morange.yOffset = 40
        morange.characterColor = NSColor(red: 1.0, green: 0.4, blue: 0.0, alpha: 1.0)

        morange.flipXOffset = -9

        morange.positionProgress = 0.5

        morange.pauseEndTime = CACurrentMediaTime() + Double.random(in: 8.0...14.0)

        characters = [morange]
        characters.forEach { $0.controller = self }
        morange.setup()

        setupDebugLine()
        startDisplayLink()
        startAudioCacheMaintenance()

        if !UserDefaults.standard.bool(forKey: Self.onboardingKey) {
            triggerOnboarding()
        }
    }

    private func startAudioCacheMaintenance() {
        cleanHermesAudioCache(reason: "启动自动清理", announce: false)
        let timer = Timer(timeInterval: 24 * 60 * 60, repeats: true) { [weak self] _ in
            self?.cleanHermesAudioCache(reason: "每日自动清理", announce: false)
        }
        audioCacheCleanupTimer = timer
        RunLoop.main.add(timer, forMode: .common)
    }

    func cleanHermesAudioCache(reason: String = "手动清理", announce: Bool = true) {
        if announce {
            characters.first?.applyPetState(.working, phrase: "收拾语音缓存", completion: true, autoReturnAfter: 2.0)
        }
        DispatchQueue.global(qos: .utility).async {
            let report = HermesBridge.shared.cleanupAudioCaches()
            if report.hasChanges || announce {
                HermesBridge.shared.recordPlanRequest("Hermes 语音缓存清理", detail: "\(reason)：\(report.summary)")
                HermesBridge.shared.appendMemoryEvent(category: "语音缓存", title: reason, detail: report.summary)
            }
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                if announce {
                    let state: WalkerCharacter.PetState = report.failures.isEmpty ? .done : .confused
                    self.characters.first?.applyPetState(
                        state,
                        phrase: report.failures.isEmpty ? "缓存收好啦" : "有点没扫干净",
                        completion: true,
                        autoReturnAfter: 3.0
                    )
                    self.characters.first?.terminalView?.appendToolResult(summary: report.summary, isError: !report.failures.isEmpty)
                } else if report.hasChanges {
                    self.characters.first?.terminalView?.appendToolResult(summary: "自动清理语音缓存：\(report.summary)", isError: !report.failures.isEmpty)
                }
            }
        }
    }

    private func triggerOnboarding() {
        guard let morange = characters.first else { return }
        morange.isOnboarding = true
        // Show "hi!" bubble after a short delay so the character is visible first
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            guard morange.window != nil else { return }
            morange.currentPhrase = "hi!"
            morange.showingCompletion = true
            morange.completionBubbleExpiry = CACurrentMediaTime() + 600
            morange.showBubble(text: "hi!", isCompletion: true)
            morange.playCompletionSound()
        }
    }

    func completeOnboarding() {
        UserDefaults.standard.set(true, forKey: Self.onboardingKey)
        characters.forEach { $0.isOnboarding = false }
    }

    // MARK: - Debug

    private func setupDebugLine() {
        let win = NSWindow(contentRect: CGRect(x: 0, y: 0, width: 100, height: 2),
                           styleMask: .borderless, backing: .buffered, defer: false)
        win.isOpaque = false
        win.backgroundColor = NSColor.red
        win.hasShadow = false
        win.level = NSWindow.Level(rawValue: NSWindow.Level.statusBar.rawValue + 10)
        win.ignoresMouseEvents = true
        win.collectionBehavior = [.canJoinAllSpaces, .stationary]
        win.orderOut(nil)
        debugWindow = win
    }

    private func updateDebugLine(dockX: CGFloat, dockWidth: CGFloat, dockTopY: CGFloat) {
        guard let win = debugWindow, win.isVisible else { return }
        win.setFrame(CGRect(x: dockX, y: dockTopY, width: dockWidth, height: 2), display: true)
    }

    // MARK: - Dock Geometry

    private func getDockIconArea(screenWidth: CGFloat) -> (x: CGFloat, width: CGFloat) {
        let dockDefaults = UserDefaults(suiteName: "com.apple.dock")
        let tileSize = CGFloat(dockDefaults?.double(forKey: "tilesize") ?? 48)
        // Each dock slot is the icon + padding. The padding scales with tile size.
        // At default 48pt: slot ≈ 58pt. At 37pt: slot ≈ 47pt. Roughly tileSize * 1.25.
        let slotWidth = tileSize * 1.25

        let persistentApps = dockDefaults?.array(forKey: "persistent-apps")?.count ?? 0
        let persistentOthers = dockDefaults?.array(forKey: "persistent-others")?.count ?? 0

        // Only count recent apps if show-recents is enabled
        let showRecents = dockDefaults?.bool(forKey: "show-recents") ?? true
        let recentApps = showRecents ? (dockDefaults?.array(forKey: "recent-apps")?.count ?? 0) : 0
        let totalIcons = persistentApps + persistentOthers + recentApps

        var dividers = 0
        if persistentApps > 0 && (persistentOthers > 0 || recentApps > 0) { dividers += 1 }
        if persistentOthers > 0 && recentApps > 0 { dividers += 1 }
        // show-recents adds its own divider
        if showRecents && recentApps > 0 { dividers += 1 }

        let dividerWidth: CGFloat = 12.0
        var dockWidth = slotWidth * CGFloat(totalIcons) + CGFloat(dividers) * dividerWidth

        let magnificationEnabled = dockDefaults?.bool(forKey: "magnification") ?? false
        if magnificationEnabled,
           let largeSize = dockDefaults?.object(forKey: "largesize") as? CGFloat {
            // Magnification only affects the hovered area; at rest the dock is normal size.
            // Don't inflate the width — characters should stay within the at-rest bounds.
            _ = largeSize
        }

        // Small fudge factor for dock edge padding
        dockWidth *= 1.1
        let dockX = (screenWidth - dockWidth) / 2.0
        return (dockX, dockWidth)
    }

    // MARK: - Display Link

    private func startDisplayLink() {
        let screen = activeScreen ?? NSScreen.main ?? NSScreen.screens.first
        displayLink = screen?.displayLink(target: self, selector: #selector(displayLinkTick(_:)))
        displayLink?.add(to: .main, forMode: .common)
    }

    @objc private func displayLinkTick(_ link: CADisplayLink) {
        tick()
    }

    var activeScreen: NSScreen? {
        if pinnedScreenIndex >= 0, pinnedScreenIndex < NSScreen.screens.count {
            return NSScreen.screens[pinnedScreenIndex]
        }
        return pointerScreen
    }

    /// The dock lives on the screen where visibleFrame.origin.y > frame.origin.y (bottom dock)
    /// On screens without the dock, visibleFrame.origin.y == frame.origin.y
    private func screenHasDock(_ screen: NSScreen) -> Bool {
        return screen.visibleFrame.origin.y > screen.frame.origin.y
    }

    func tick() {
        guard let screen = activeScreen else { return }

        let screenWidth = screen.frame.width
        let dockX: CGFloat
        let dockWidth: CGFloat
        let dockTopY: CGFloat

        if screenHasDock(screen) {
            // Dock is on this screen — constrain to dock area
            (dockX, dockWidth) = getDockIconArea(screenWidth: screenWidth)
            dockTopY = screen.visibleFrame.origin.y
        } else {
            // No dock on this screen — use full screen width with small margin
            let margin: CGFloat = 40.0
            dockX = screen.frame.origin.x + margin
            dockWidth = screenWidth - margin * 2
            dockTopY = screen.frame.origin.y
        }

        updateDebugLine(dockX: dockX, dockWidth: dockWidth, dockTopY: dockTopY)

        characters.forEach { $0.repairVisibilityIfNeeded() }
        let activeChars = characters.filter { $0.window != nil && $0.window.isVisible }

        let now = CACurrentMediaTime()
        let anyWalking = activeChars.contains { $0.isWalking }
        for char in activeChars {
            if char.isIdleForPopover { continue }
            if char.isPaused && now >= char.pauseEndTime && anyWalking {
                char.pauseEndTime = now + Double.random(in: 5.0...10.0)
            }
        }
        for char in activeChars {
            char.update(dockX: dockX, dockWidth: dockWidth, dockTopY: dockTopY)
        }

        let sorted = activeChars.sorted { $0.positionProgress < $1.positionProgress }
        for (i, char) in sorted.enumerated() {
            char.window.level = NSWindow.Level(rawValue: WalkerCharacter.characterWindowLevel.rawValue + i)
        }
    }

    deinit {
        displayLink?.invalidate()
        audioCacheCleanupTimer?.invalidate()
    }
}
